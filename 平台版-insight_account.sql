...
-- ----------------------------
-- Table structure for iab_account
-- ----------------------------
DROP TABLE IF EXISTS `iab_account`;
CREATE TABLE `iab_account` (
  `id` char(32) NOT NULL COMMENT '主键(UUID),同租户ID',
  `balance` decimal(18, 2) NOT NULL DEFAULT '0.00' COMMENT '账户余额',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci ROW_FORMAT=COMPACT COMMENT='商户账户表';

-- ----------------------------
-- Table structure for iab_account_check
-- ----------------------------
DROP TABLE IF EXISTS `iab_account_check`;
CREATE TABLE `iab_account_check` (
  `id` char(32) NOT NULL COMMENT '主键(UUID)',
  `account_id` char(32) NOT NULL COMMENT '商户账户ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci ROW_FORMAT=COMPACT COMMENT='商户账户对账单表';

-- ----------------------------
-- Table structure for iab_account_detail
-- ----------------------------
DROP TABLE IF EXISTS `iab_account_detail`;
CREATE TABLE `iab_account_detail` (
  `id` char(32) NOT NULL COMMENT '主键(UUID)',
  `account_id` char(32) NOT NULL COMMENT '用户账户ID',
  `check_id` char(32) DEFAULT NULL COMMENT '对账单ID',
  `type` tinyint(3) NOT NULL COMMENT '类型:4.订单退款;5.售后赔付;7.订单收入;8.提现',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci ROW_FORMAT=COMPACT COMMENT='商户账户流水表';
...
CREATE TABLE `iau_account_detail` (
  `id` char(32) NOT NULL COMMENT '主键(UUID)',
  `account_id` char(32) NOT NULL COMMENT '用户账户ID',
  `type` tinyint(3) NOT NULL COMMENT '类型:0.订单收银;1.充值;4.订单退款;5.售后赔付;7.订单扣款',
  `business_code` varchar(32) DEFAULT NULL COMMENT '业务单号',
  `amount` decimal(18, 2) NOT NULL COMMENT '流水金额',
  `remark` varchar(1024) DEFAULT NULL COMMENT '备注',
  `created_time` datetime NOT NULL COMMENT '创建时间',
